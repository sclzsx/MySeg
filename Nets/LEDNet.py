...
class LEDNet(nn.Module):
    def __init__(self, classes, divisor=1):
        super().__init__()
        self.initial_block = DownsamplerBlock(3, 32//divisor)

        self.layers = nn.ModuleList()

        for x in range(0, 3):
            self.layers.append(SS_nbt_module_paper(32//divisor, 0.03, 1))

        self.layers.append(DownsamplerBlock(32//divisor, 64//divisor))

        for x in range(0, 2):
            self.layers.append(SS_nbt_module_paper(64//divisor, 0.03, 1))

        self.layers.append(DownsamplerBlock(64//divisor, 128//divisor))

        for x in range(0, 1):
            self.layers.append(SS_nbt_module_paper(128//divisor, 0.3, 1))
            self.layers.append(SS_nbt_module_paper(128//divisor, 0.3, 2))
            self.layers.append(SS_nbt_module_paper(128//divisor, 0.3, 5))
            self.layers.append(SS_nbt_module_paper(128//divisor, 0.3, 9))

        for x in range(0, 1):
            self.layers.append(SS_nbt_module_paper(128//divisor, 0.3, 2))
            self.layers.append(SS_nbt_module_paper(128//divisor, 0.3, 5))
            self.layers.append(SS_nbt_module_paper(128//divisor, 0.3, 9))
            self.layers.append(SS_nbt_module_paper(128//divisor, 0.3, 17))

        self.apn = APNModule(in_ch=128//divisor, out_ch=classes)
...
#     model = LEDNet(classes=2).to(device)
#     summary(model, (3, 528, 960))
if __name__ == '__main__':
